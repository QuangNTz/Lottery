		...
		<div class="tab-content flex-column justify-content-between">
				...
				<div class="result-sheet">
					...
					<table class="table table-hover">
						...
						<tbody class="text-center align-middle">
							<c:forEach items="${sessionScope.rsList}" var="o">
									...
									<td>
										<div class="btn-group btn-group-justified">
											...
											<a class="btn btn-danger" href="/Lottery/deleteRS?id=${o.lottery_id}" 
												onclick="return confirm('Bạn có muốn vé dò ${o.lottery_com} ngày <fmt:formatDate pattern = "dd-MM-yyyy" value = "${l_date}"/>?')">
			...
			<div class="pages-nav">
				<form class="d-flex" action="search_resultsheetdata">
					<table>
						<tr>
							<td><input type="search" class="form-control me-2" name="rs_search" placeholder="Cty xổ số" aria-label="Search"></td>							
							<td><button type="submit" class="btn btn-outline-info"><i class="fa-solid fa-magnifying-glass"></i></button></td>
						</tr>						
					</table>						
				</form>
				<ul class="pagination justify-content-center">
					<c:forEach begin="1" end="${sessionScope.rsMap.endPage}" var="i">
						<li class="page-item ${sessionScope.rsMap.activePage == i ? 'active' : '' }" aria-current="${sessionScope.rsMap.activePage == i ? 'page' : '' }">
							<a class="page-link" href="/Lottery/loadresultsheetdata?page=${i}">${i}</a>
						</li>
					</c:forEach>
				</ul>
			</div>
