...
<div id="main-content" class="row justify-content-center">
	...
	<div class="middle-content col-md-7 col-sm-12">
		...
		<ul class="nav nav-tabs">
			<li class="nav-item">
				<a class="nav-link active" href="/Lottery/loadresultsheetdata">Add Result sheet</a>
			</li>
		</ul>
		
		<div class="tab-content">
			<form class="needs-validation" action="add_resultsheet" method="post" novalidate>
				<div class="input-group mb-3" style="min-width: 250px">
					<span class="input-group-text" id="basic-addon1">Tỉnh:</span>
					<select class="form-select" name="lottery_com" aria-label="Default select example" required>
						<option selected disabled value="">--Chọn tỉnh--</option>
						<c:forEach items="${sessionScope.lcList}" var="o">
							<option value="${o}" ${lottery_com == o ? 'selected' : ''}>${o}</option>
						</c:forEach>
					</select>
				</div>
				<div class="input-group mb-3" style="min-width: 250px">					
					<span class="input-group-text" id="basic-addon1">Ngày:</span>
					<input type="text" id="datepicker" class="form-control" size="30" name="lottery_date" value="${lottery_date}" required readonly>
				...
				<div class="btn-group">
					<div>
						<button type="submit" class="btn btn-info btn-addRS">Thêm vé dò</button>
						<input type="reset" class="btn btn-warning" value="Reset">
						<a class="btn btn-warning" href="/Lottery/loadresultsheetdata">Hủy</a>						
					</div>
				</div>
