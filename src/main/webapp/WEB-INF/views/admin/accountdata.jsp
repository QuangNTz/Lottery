		...
		<div class="tab-content flex-column justify-content-between">
				...
				<div class="user-info">
					...
					<table class="table table-hover">
						...
						<tbody class="text-center align-middle">
							<c:forEach items="${sessionScope.accList}" var="o">
								<tr>
									<td>
										<div class="form-check">
											<input type="checkbox" class="form-check-input select-item" name="id" 
												value="${o.user_mail}" ${(o.account_role==1 || o.user_status == 0) ? 'disabled' : '' }>
										</div>
									</td>
									<td>${o.user_mail}</td>
									<td>*****</td>
			...
			<div class="pages-nav">				
				<form class="d-flex" action="search_accountdata">
					<table>
						<tr>					      	
							<td><input type="search" class="form-control me-2" name="usr_search" placeholder="Tên tkhoản - Số đthoại" aria-label="Search"></td>
				...
				<ul class="pagination justify-content-center">
					<c:forEach begin="1" end="${sessionScope.accMap.endPage}" var="i">
						<li class="page-item ${sessionScope.accMap.activePage == i ? 'active' : '' }" aria-current="${sessionScope.accMap.activePage == i ? 'page' : '' }">
							<a class="page-link" href="/Lottery/loadaccountdata?page=${i}">${i}</a>
						</li>
					</c:forEach>
				</ul>				
			</div>
